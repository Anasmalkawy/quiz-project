...
function addQuestionData(obj, count) {
  // displays the question title in the .quiz-area.
  if (currentIndex < count) {
    // It loops to create 4 answer options and appends them as radio buttons for user selection.
...
function saveAnswerToLocalStorage(question) {
  ...
  let answers = document.getElementsByName("question");
  let theChoosenAnswer;
// here will put the answer the user check in theChoosenanswer 
  for (let i = 0; i < answers.length; i++) {
    if (answers[i].checked) {
      theChoosenAnswer = answers[i].dataset.answer;
    }
